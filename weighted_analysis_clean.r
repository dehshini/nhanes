...
############################################################
# Calculate the proportions for high SRH
...
highsrh_priv_insurance <- lapply(
    weighted_nhanes_list, subset, insurance == 1
)
# calculate the proportion of high_srh
prop_highsrh_priv_insurance <- do.call(
    rbind, lapply(highsrh_priv_insurance, calculate_svy_prop, "high_srh")
)
# add a column for insurance
prop_highsrh_priv_insurance$Insurance <- "Private insurance"
write.csv(
    prop_highsrh_priv_insurance,
    file = "./out/highsrh_priv_insurance.csv"
)


highsrh_pub_insurance <- lapply(
    weighted_nhanes_list, subset, insurance == 2
)
# calculate the proportion of high_srh
prop_highsrh_pub_insurance <- do.call(
    rbind, lapply(highsrh_pub_insurance, calculate_svy_prop, "high_srh")
)
# add a column for insurance
prop_highsrh_pub_insurance$Insurance <- "Public insurance"
write.csv(
    prop_highsrh_pub_insurance,
    file = "./out/highsrh_pub_insurance.csv"
)
